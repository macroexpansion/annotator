...
export default {
  ...
  methods: {
    ...
    setPreferences(pref) {
      ...
      this.polygon.completeDistance = pref.completeDistance || this.polygon.completeDistance;
    ...
    onMouseDrag(event) {
      ...
      this.polygon.path.fullySelected = false;
    },
    onMouseDown(event) {
      let wasNull = false;
    ...
    /**
     * Closes current polygon and unites it with current annotation.
     * @returns {boolean} sucessfully closes object
     */
    complete() {
      if (this.polygon.path == null) return false;
