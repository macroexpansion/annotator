...
def get_image_coco(image_id):
    ...
    for category in fix_ids(bulk_categories):
        ...
        if category_annotations.count() == 0:
            del category['keypoint_edges'], category['keypoint_labels'], \
                category['keypoint_colors'], category['metadata']
        ...
        for annotation in category_annotations:
            ...
            has_label = annotation.get('label')

            if has_segmentation or has_keypoints or has_label:
                if has_keypoints:
                    arr = np.array(annotation.get('keypoints', []))
                    arr = arr[2::3]
                    annotation['num_keypoints'] = len(arr[arr > 0])

                del annotation['keypoints'], annotation['metadata']
