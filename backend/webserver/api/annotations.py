...
create_annotation.add_argument('label', type=bool, location='json')
create_annotation.add_argument('creator', type=str, location='json')
...
class Annotation(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            annotation.save(creator=creator)
...
@api.route('/label')
class AnnotationLabel(Resource):    
    @login_required
    def post(self):
        """ Create annotation for classification """
        args = create_annotation.parse_args()
        image_id = args.get('image_id')
        category_id = args.get('category_id')
        isbbox = args.get('isbbox', False)
        ...
        annotation = current_user.annotations.filter(
            image_id=image_id,
            category_id=category_id,
            creator=creator,
            deleted=False).first()
...
class AnnotationId(Resource):
    ...
    #     annotation = current_user.annotations.filter(
    #         id=annotation_id, creator=current_user.username).first()
    ...
    def delete(self, annotation_id):
        ...
        # if annotation.creator == current_user.username:
