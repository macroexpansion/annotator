...
class ProjectImage(Resource):
    ...
    def get(self, project_id):
        ...
        dataset = current_user.datasets.filter(project_id=project_id, deleted=False).first()
        if dataset is None:
            return { 'success': False }

        image = current_user.images.filter(dataset_id=dataset.id, deleted=False).first()
        if image is None:
        ...
        if thumbnail:
            try:
                pil_image = image.open_thumbnail()
...
class ImageId(Resource):
    ...
    def delete(self, image_id):
        ...
        if not current_user.can_delete(image):
            return {"message": "You do not have permission to delete the image"}, 403
        
        # image.update(set__deleted=True, set__deleted_date=datetime.datetime.now())
        image.delete() # delete the queried image's thumbnail, annotation and its record on database
...
class ImageCoco(Resource):
    ...
    def get(self, image_id):
        ...
        image = current_user.images.filter(id=image_id, deleted=False).exclude('deleted_date').first()
