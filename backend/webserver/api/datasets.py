...
dataset_create.add_argument("project_id", required=True, location="json")
...
page_data.add_argument("order", default="id", help="Order to display images")
page_data.add_argument("project_id", help="Project id")
page_data.add_argument("user", help="user")
...
image_upload.add_argument("crop", type=bool, default=False, location="json")
...
export.add_argument(
    "user", type=str, default=None, required=True, help="user annotation"
)
...
dataset_stats = reqparse.RequestParser()
dataset_stats.add_argument(
    "keywords",
    location="json",
    type=list,
    default=[],
...
class Dataset(Resource):
    ...
    def post(self):
        ...
        if not project_id:
            return {"message": "No project id"}, 400

        project = ProjectModel.objects(id=project_id).first()
        if project is None:
            return {"message": "Project error"}
...
class DatasetData(Resource):
    ...
    def get(self):
        ...
        for dataset in datasets:
            ...
            dataset_json["userNumberAnnotated"] = images.filter(
                __raw__={f"user_annotated.{current_user.username}": True}
            ).count()
...
class DatasetsData(Resource):
    ...
    def get(self):
        ...
        datasets = current_user.datasets.filter(project_id=project_id, deleted=False)
        pagination = Pagination(datasets.count(), limit, page)
        datasets = datasets[pagination.start : pagination.end]
        ...
        return {
            ...
            "datasets": datasets_json,
            "categories": query_util.fix_ids(
                current_user.categories.filter(
...
class DatasetDataId(Resource):
    ...
    def get(self, dataset_id):
        ...
        annotated = request.args.get("annotated", None)
        ...
        if annotated is not None:
            if annotated != "no":
                if annotated == "false":
                    query_build &= Q(__raw__={f"user_annotated.{user}": {"$ne": True}})
                elif annotated == "true":
                    query_build &= Q(__raw__={f"user_annotated.{user}": {"$eq": True}})
            else:
                query_build &= Q(__raw__={f"user_annotated.{user}": {"$eq": " "}})
...
class DatasetExport(Resource):
    ...
    def get(self, dataset_id):
        ...
        categories = args.get("categories")
        user = args.get("user", current_user.username)
        ...
        return dataset.export_coco(categories=categories, user=user)
...
class ImageUpload(Resource):
    ...
    def post(self, dataset_id):
        ...
        return dataset.upload_images(
            images, upload_id=upload_id, scan_id=scan_id, crop=crop
        )


@api.route("/<int:dataset_id>/taskids")
class ImageUp(Resource):
    @login_required
    def get(self, dataset_id):
        dataset = DatasetModel.objects(id=dataset_id).first()

        return dataset.get_task_id()


@api.route("/<int:dataset_id>/folders")
@api.expect(image_upload)
class ImageUpload(Resource):
    @login_required
    def post(self, dataset_id):
